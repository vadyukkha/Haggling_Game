...
Offer create_offer_ball(int cost_hat, int cost_book, int cost_ball)  {
    Offer offer;
    offer.hat = 1;
    offer.book = 2;
    offer.ball = 0;
...
Offer decide_on_offer_ball(const Offer& offer, int cost_hat, int cost_book, int cost_ball) {
    Offer new_offer;
    if (offer.ball > 2) {
        new_offer.ball = 200;
        return new_offer;
    }
    return create_offer_ball(cost_hat, cost_book, cost_ball);
}
...
extern "C" EXPORT int decide(int turn, int cost_hat, int cost_book, int cost_ball, int count_hat, int count_book, int count_ball) {
    int a = -1e9;
    if (turn == 0) {
        Offer offer = create_offer_ball(cost_hat, cost_book, cost_ball);
        ...
        Offer answer = decide_on_offer_ball(enemy_offer, cost_hat, cost_book, cost_ball);
